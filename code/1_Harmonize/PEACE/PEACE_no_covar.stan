    ...
    for (i in 1:I) {
        ...
        if (this_batch==1) {
            target += log_mix(phi[this_batch],
                normal_lpdf(y[i] | alpha[1], sigma[1]),
                normal_lpdf(y[i] | alpha[2], sigma[2]));
        } else {
            target += log_mix(phi[this_batch],
                normal_lpdf(y[i] | alpha[1] + gamma[this_batch-1], delta_sigma1[this_batch-1]),
                normal_lpdf(y[i] | alpha[2] + gamma[this_batch-1], delta_sigma2[this_batch-1]));
        }
    }
}
    ...
    array[I,K] real y_combat ;
    {
        ...
        for (i in 1:I) {
            ...
            if (this_batch==1) {
                ...
                alpha1_beta_x = alpha[1];
                alpha2_beta_x = alpha[2];
