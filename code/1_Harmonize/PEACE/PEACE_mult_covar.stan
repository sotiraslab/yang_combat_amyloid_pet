...
transformed data {
    ...
    vector[K] inv_s_y ;
    array[M] real s_x ;
    array[M] vector[K] multiplier_s_y_s_x ;
    for (k in 1:K) {
        m_y[k] = mean(y[1:I,k]);
        s_y[k] = sd(y[1:I,k]);
    }
    ...
    for (m in 1:M) {
        s_x[m] = sd(x[1:I,m]);
        multiplier_s_y_s_x[m] = mtplr * s_y ./ s_x[m] ;
    }
}
parameters {
    array[B] real<lower=0, upper=1> phi;
    array[Z] vector[K] alpha;
    array[Z,M] vector[K] beta;
...
model {
    ...
    for (z in 1:Z) {
        ...
        for (m in 1:M) {
            beta[z,m] ~ normal(0, multiplier_s_y_s_x[m]);
        }
        sigma[z] ~ exponential(inv_s_y);
    }
    for (i in 1:I) {
        int this_batch = batch[i];
        vector[K] beta_x1 = rep_vector(0,K);
        vector[K] beta_x2 = rep_vector(0,K);
        for (m in 1:M) {
            beta_x1 += beta[1,m] * x[i,m];
            beta_x2 += beta[2,m] * x[i,m];
        }
        if (this_batch==1) {
            target += log_mix(phi[this_batch],
                normal_lpdf(y[i] | alpha[1] + beta_x1, sigma[1]),
                normal_lpdf(y[i] | alpha[2] + beta_x2, sigma[2]));
        } else {
            target += log_mix(phi[this_batch],
                normal_lpdf(y[i] | alpha[1] + beta_x1 + gamma[this_batch-1], delta_sigma1[this_batch-1]),
                normal_lpdf(y[i] | alpha[2] + beta_x2 + gamma[this_batch-1], delta_sigma2[this_batch-1]));
        }
    }
}
    ...
    array[I,K] real y_combat ;
    {
        ...
        for (i in 1:I) {
            ...
            if (this_batch==1) {
                ...
                alpha1_beta_x = alpha[1] + beta_x1;
                alpha2_beta_x = alpha[2] + beta_x2;
